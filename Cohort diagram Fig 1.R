...
residents <- enclave.total %>%    
 ...
 c<- c +
   ...
   annotate('text', x= 100, y=135,  size=2.5, hjust=0.5, vjust=0.5,
            label= str_wrap(paste0(prettyNum(avail.wt, big.mark =  ","), " Patients with weight measurement within 2 years"),
                            indent =0 , width =60, exdent = 0))  + 
 ...
 other.tx <- enclave.total %>%    
   ...
   filter(is.na(bebtelovimab_date) |  bebtelovimab_date >  covid_date+30) %>%      # remove those that received bebtelovimab within 30d of covid diagnosis
   filter(outpatient.remdesivir == 0) %>%
   filter(is.na(paxlovid_date) |  paxlovid_date <  covid_date+2) %>% # remove those that nirmatrelvir beyond 2d of covid diagnosis
   count()
 ...
 c<- c +
   ...
   annotate('text', x= 100, y=105,  size=2.5, hjust=0.5, vjust=0.5,
            label= str_wrap(paste0(prettyNum(other.tx, big.mark =  ","), " Patients not receiving other outpatient treatments for early COVID-19"),
                            indent =0 , width =60, exdent = 0)) + 
   
   geom_rect(xmin = 125, xmax=182, ymin=112, ymax=128, color='black',
             fill='white', size=0.25) +
   annotate('text', x= 127, y=120,  size=2.5, hjust=0, vjust=0.5,
            label= str_wrap(paste0(prettyNum(avail.wt - other.tx1, big.mark =  ","), " Patients received anti-SARS-CoV-2 monoclonal antibodies, molnupirivir, or outpatient remdesivir\n", "\n",
                                   prettyNum(other.tx1 - other.tx, big.mark =  ","), " Patients prescribed nirmatrelvir more than 2 calendar days after diagnosis"),
                            indent =0 , width =38, exdent = 0)) +
   geom_segment(
 ...
 # box of eligible if outpatients
 ...
 c<- c +
   ...
   annotate('text', x= 127, y=90,  size=2.5, hjust=0, vjust=0.5,
            label= str_wrap(paste0(prettyNum(other.tx - eligible, big.mark =  ","), " Patients with eGFR < 30 mL/min or receiving medications not advised to be coadministered"),
                            indent =0 , width =38, exdent = 0)) +
   geom_segment(
 ...
 # box of eligible outpatients
 eligible.outpatient <- enclave.total %>%    
   filter(covid_date >= as.Date("2022-01-01") & covid_date <= as.Date("2022-07-17")) %>%
   filter(age.at.covid >= 50) %>%
   filter(state == "Massachusetts" | state == "New Hampshire") %>%
   filter(!is.na(bmi)) %>%
 ...
 c<- c +
   ...
   annotate('text', x= 100, y=45,  size=2.5, hjust=0.5, vjust=0.5,
            label= str_wrap(paste0(prettyNum( eligible.outpatient, big.mark =  ","), " Patients diagnosed as outpatients and eligible for nirmatrelvir plus ritonavir"),
                            indent =0 , width =60, exdent = 0)) + 
   
   geom_rect(xmin = 125, xmax=182, ymin=55, ymax=65, color='black',
             fill='white', size=0.25) +
   annotate('text', x= 127, y=60,  size=2.5, hjust=0, vjust=0.5,
            label= str_wrap(paste0(prettyNum(eligible -  eligible.outpatient, big.mark =  ","), " Patients diagnosed within one calendar day of admission or death"),
                            indent =0 , width =38, exdent = 0)) +
   geom_segment(
